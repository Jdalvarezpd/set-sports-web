...
function actualizarTabla($conexion){
		$sentencia = $conexion->prepare("SELECT * FROM partidos WHERE id_evento= 61 AND etapa = 'Grupos' AND grupo = 'B'");
		$sentencia->execute();
		$partidos = $sentencia->fetchAll();

		//PRIMERO VACIAR LA TABLA DE LIGA
			$vaciar = $conexion->prepare(
				"UPDATE tabla_liga SET pj = :pj, pg = :pg, pe = :pe, pp = :pp, gf = :gf, gc = :gc, dif = :dif, pts = :pts WHERE id_evento = 61 AND grupo = 'B' ");
