...
function actualizarTabla($conexion){
		$sentencia = $conexion->prepare("SELECT * FROM partidos WHERE id_evento= 60 AND etapa = 'Grupos'");
		$sentencia->execute();
		...
		for ($i=0; $i < count($partidos); $i++) {
			...
			if($partidos[$i]['r1'] > $partidos[$i]['r2']){
				...
				$sentencia = $conexion->prepare("SELECT * FROM tabla_liga WHERE id_evento= 60 AND equipo = '". $partidos[$i]['equipo_uno'] ."' ");
				$sentencia->execute();
				$tabla = $sentencia->fetchAll();

				$sentencia2 = $conexion->prepare("SELECT * FROM tabla_liga WHERE id_evento= 60 AND equipo = '". $partidos[$i]['equipo_dos'] ."' ");
				$sentencia2->execute();
